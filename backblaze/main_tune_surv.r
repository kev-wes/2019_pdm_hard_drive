...
for (model in models)
{
  ...
  train[['failure']] = as.numeric(train[['failure']])
  test <- R_read_pickle_file(paste(data_path, "data\\rawdata\\data_test_", model, ".pkl", sep=""))
  test[['failure']] = as.numeric(test[['failure']])
  # Train statistics
  train_length = nrow(train)
  train_serials = sapply(train['serial_number'], function(x) length(unique(x)))
  # Load variable importance
  if(file_test("-f", paste(data_path, "data\\rawdata\\vimp_", model,".csv", sep=""))){
    # Train simple RF 
    #x = str(train[, !names(train) %in% c("date", "failure", "capacity_bytes", "serial_number")])
    rfr = rfsrc.fast(RUL ~ ., data = train[, !names(train) %in% c("date", "RUL", "capacity_bytes", "serial_number", "model")],
  ...
  vimp.top = c(vimp.top, "failure")
  # Fit & tune random forest
  rfr = tune(Surv(Days_In_Service, failure) ~ ., data = train[, names(train) %in% vimp.top],
             trace = TRUE, ntreeTry=ntree)
  log = read.csv(file=paste(data_path, "data\\rawdata\\my_models.csv", sep=""), sep=";", stringsAsFactors=FALSE)
  id = tail(log, 1)$ID + 1
  par(mar = rep(2, 4))
  plot.survival(rfr$rf)
  predi = predict(rfr$rf, data = test[, names(train) %in% vimp.top])
  ...
  # Test forest
  y_pred = predict(rfr$rf,test[, !names(test) %in% c("date", "failure", "capacity_bytes", "serial_number")])$predicted
  y_test = test$RUL
